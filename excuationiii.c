...
/*   Updated: 2024/02/07 23:08:20 by fmaqdasi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child(char **argv, char **envp, int **pipe_fd, int i[2])
{
	...
	fd = open(argv[i[0] - 2], O_RDONLY);
	if (fd == -1)
	{
		ft_putstr_fd(strerror(errno), 2);
		write(2, "\n", 1);
		return (close_pipe(pipe_fd, i[1]), free_pipe(pipe_fd), exit(3));
	}
	if (dup2(fd, STDIN_FILENO) == -1 || dup2(pipe_fd[i[0] - 3][1],
		STDOUT_FILENO) == -1)
	{
	...
	close(fd);
	if (excute_command(argv, envp, i[0] - 1) == -1)
